...
//    List<Order> orders_without_details(@Param("date") Calendar date);
    ...
    @Query(value = "select * from orders where not exists (select from detail where orders.date > '2016-09-06' or detail.ord_id = orders.id  )", nativeQuery = true)
    List<Order> orders_without_details();


    @Query(value = "select c.id as customer_id, c.name as customer_name, o.date as order_date from customer c left join orders o on o.cust_id = c.id where concat(cust_id, date) IN (select concat(cust_id, max(date)) from orders group by cust_id)", nativeQuery = true)
    List<Customers_last_orders_projection> customers_last_orders();


    @Query(value = "select o.id as orderId, o.date as orderDate, d.quantity as quantity, p.price * quantity as total_price from orders o inner join detail d on o.id = d.ord_id inner join product p on d.pr_id = p.id and p.description is not null where not exists (select from invoice i where i.ord_id = o.id)", nativeQuery = true)
